...
import type { Registro, NovedadRetiroEmpleadorFormData } from "../types/novedad-retiro-empleador-types"
...
const EXCEL_COLUMN_MAPPING = {
  ...
  'MUNICIPIO': 'municipio',
  'FECHA_RETIRO_EMPLEADOR': 'fecha_retiro_empleador',
  'CAUSAL_RETIRO_EMPLEADOR': 'causal_retiro_empleador',
} as const;

type FormFieldKeys = (typeof EXCEL_COLUMN_MAPPING)[keyof typeof EXCEL_COLUMN_MAPPING];

interface NovedadRetiroEmpleadorMassiveUploadProps {
  trigger?: React.ReactNode;
  onSuccess?: (result: ProcessingResult) => void;
  onError?: (error: string) => void;
}

export function NovedadRetiroEmpleadorMassiveUpload({ trigger, onSuccess, onError }: NovedadRetiroEmpleadorMassiveUploadProps) {
  ...
  const processRetiroData = useCallback(
    async (data: any[], setProgress: (progress: number) => void): Promise<ProcessingResult> => {
      ...
      for (let i = 0; i < data.length; i++) {
        ...
        try {
          const formData: Partial<NovedadRetiroEmpleadorFormData> = {};
          ...
          Object.keys(row).forEach(originalKey => {
            ...
            if (formField) {
              ...
              if (formField === 'fecha_retiro_empleador') {
          ...
          const sanitizedData = sanitizeFormData(formData as NovedadRetiroEmpleadorFormData);
          ...
          Object.entries(novedadRetiroEmpleadorValidationRules).forEach(([fieldName, rules]) => {
  ...
  const validateFileStructure = useCallback((headers: string[]) => {
    ...
    const requiredColumnsNormalized = [
      ...
      normalizeHeader('FECHA_RETIRO_EMPLEADOR'),
      normalizeHeader('CAUSAL_RETIRO_EMPLEADOR'),
    ];
  ...
  const config: MassiveUploadConfig = {
    ...
    title: "Carga Masiva de Novedad Retiro Empleador",
    instructions: [
      'Seleccione un archivo Excel (.xlsx) con la hoja "DATOS" con el formato correcto para novedad retiro empleador.',
