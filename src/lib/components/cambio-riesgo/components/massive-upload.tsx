...
import { cambioRiesgoValidationRules, sanitizeFormData } from "../validations/validation-rules"
...
interface CambioRiesgoMassiveUploadProps {
  trigger?: React.ReactNode
  onSuccess?: (result: ProcessingResult) => void
  onError?: (error: string) => void
}

export function CambioRiesgoMassiveUpload({ trigger, onSuccess, onError }: CambioRiesgoMassiveUploadProps) {
  const { agregarRegistro } = useRegistroStore()

  const processCambioRiesgoData = useCallback(
    async (data: any[], setProgress: (progress: number) => void): Promise<ProcessingResult> => {
      ...
      for (let i = 0; i < data.length; i++) {
        ...
        try {
          const formData: Partial<CambioRiesgoFormData> = {}
          const originalFormData: Partial<CambioRiesgoFormData> = {}
          ...
          Object.entries(EXCEL_COLUMN_MAPPING).forEach(([excelColumn, formField]) => {
            ...
            originalFormData[formField as keyof CambioRiesgoFormData] = 
              value !== undefined && value !== null ? String(value).trim() : ""
            formData[formField as keyof CambioRiesgoFormData] = 
              value !== undefined && value !== null ? String(value).trim() : ""
          })
          ...
          Object.keys(row).forEach(columnName => {
            ...
            if (formField && !formData[formField as keyof CambioRiesgoFormData]) {
          ...
          const sanitizedData = sanitizeFormData(formData as CambioRiesgoFormData)
          ...
          Object.entries(cambioRiesgoValidationRules).forEach(([fieldName, rules]) => {
          ...
          if (fieldErrors.length === 0) {
            ...
            const enhancedErrors = fieldErrors.map((error) => {
              ...
              const actualValue = formData[fieldName as keyof CambioRiesgoFormData]
              const displayValue =
  ...
  const config: MassiveUploadConfig = {
    ...
    processData: processCambioRiesgoData,
    getContactInfo,
