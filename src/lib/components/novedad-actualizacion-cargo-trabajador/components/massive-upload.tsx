...
import { novedadActualizacionCargoTrabajadorValidationRules, sanitizeFormData } from "../validations/validation-rules";
...
interface NovedadActualizacionCargoTrabajadorMassiveUploadProps {
  trigger?: React.ReactNode;
  onSuccess?: (result: ProcessingResult) => void;
  onError?: (error: string) => void;
}

export function NovedadActualizacionCargoTrabajadorMassiveUpload({ trigger, onSuccess, onError }: NovedadActualizacionCargoTrabajadorMassiveUploadProps) {
  ...
  const processIndependienteData = useCallback(
    async (data: any[], setProgress: (progress: number) => void): Promise<ProcessingResult> => {
      ...
      for (let i = 0; i < data.length; i++) {
        ...
        try {
          const formData: Partial<NovedadActualizacionCargoTrabajadorFormData> = {};
          ...
          const sanitizedData = sanitizeFormData(formData as NovedadActualizacionCargoTrabajadorFormData);
          ...
          Object.entries(novedadActualizacionCargoTrabajadorValidationRules).forEach(([fieldName, rules]) => {
