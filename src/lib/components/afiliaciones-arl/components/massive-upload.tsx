  ...
  const processARLData = useCallback(
    async (data: any[], setProgress: (progress: number) => void): Promise<ProcessingResult> => {
      ...
      for (let i = 0; i < data.length; i++) {
        ...
        try {
          const formData: Partial<ARLFormData> = {}
          const originalFormData: Partial<ARLFormData> = {}
          ...
          Object.entries(EXCEL_COLUMN_MAPPING).forEach(([excelColumn, formField]) => {
            ...
            originalFormData[formField as keyof ARLFormData] = value !== undefined && value !== null ? String(value).trim() : ""
            ...
            formData[formField as keyof ARLFormData] = value ? String(value).trim() : ""
          })
          ...
          Object.keys(row).forEach(columnName => {
            ...
            if (formField && !formData[formField as keyof ARLFormData]) {
          ...
          const sanitizedData = sanitizeFormData(formData as ARLFormData)
          ...
          if (fieldErrors.length === 0) {
            ...
            const enhancedErrors = fieldErrors.map((error) => {
              ...
              const actualValue = formData[fieldName as keyof ARLFormData]
              const displayValue =
